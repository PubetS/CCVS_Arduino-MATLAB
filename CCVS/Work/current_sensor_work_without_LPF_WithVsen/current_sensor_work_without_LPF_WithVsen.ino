...
void loop()
{
  ...
  while (1)
  {
    ...
    Isensor = (IRawValue / 1024.0) * 5000.0; // Gets you mV from mA
    VRawValue = analogRead(VanalogIn);
    Vsensor = (VRawValue / 1024.0) * 5000.0; // Gets you mV

//    for (int i = 1; i < 100; i++)
//    {
//      IRawValue = analogRead(IanalogIn);
//      Isensor = (IRawValue / 1024.0) * 5000.0;
...
//    }
   ...
   // Vsensor = filV;
    ...
    V_out_cal = mV_out_cal / 1000.0;
//    pluseinitial = (V_out_cal * 255) / 12;
//
//    analogWrite(out, pluseinitial);


    if (mV_real > mV_out_cal + 2)
    {
      pluse = pluse - 1;
      if (pluse <= 0) pluse = 0;
    }
    ...
    if (mV_real <= mV_out_cal + 2 | mV_real >= mV_out_cal - 2)
    {
      pluse = pluse;
    }
    //if (pluseinitial>pluse+5|pluseinitial<pluse-5)
    //{
    //  analogWrite(out, pluseinitial);
    //}
    //else
   ...
   // pluseinitial = pluse;
    ...
    Serial.print("\t VRawValue = "); //
    Serial.println(VRawValue);
