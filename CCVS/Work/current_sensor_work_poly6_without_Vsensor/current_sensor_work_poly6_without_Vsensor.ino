...
void loop()
{
  ...
  while (1)
  {
    ...
    poly6 = p1 * (V_out * V_out * V_out * V_out * V_out * V_out)
            + p2 * (V_out * V_out * V_out * V_out * V_out) + p3 * (V_out * V_out * V_out * V_out)
            + p4 * (V_out * V_out * V_out) + p5 * (V_out * V_out) + p6 * V_out + p7;

    pluse = map( poly6, 0, 100, 0, 255);
