...
void loop(){  
  float RawValue = analogRead(CurrentSensor); //No smooting

  float SmootRawValue10 = as.smooth(RawValue); // Smoothing with window size 10

  float SmootRawValue100 = as100.analogReadSmooth(RawValue);  // Smoothing with window size 100
